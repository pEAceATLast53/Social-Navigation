...
from env.pysocialforce.potentials import PedPedPotential, PedSpacePotential
from env.pysocialforce.fieldofview import FieldOfView
from env.pysocialforce.utils import Config, stateutils, logger
...
class DesiredForce(Force):
    ...
    def init(self, scene, config):
        ...
        self.factor = np.clip(np.random.normal(1.0, 0.2, self.peds.size()), 0.8, 1.2)
        self.relaxation_time = np.clip(np.random.normal(0.5, 0.2, self.peds.size()), 0.3, 0.7)
        self.factor[-1] = 1.0
        self.relaxation_time[-1] = 0.5
    ...
    def _get_force(self):
        ...
        #force = direction * self.peds.max_speeds.reshape((-1,1)) - vel.reshape((-1,2))
        #force /= relexation_time
        force = np.divide(force, np.stack([self.relaxation_time, self.relaxation_time], -1))
        return np.multiply(force, np.stack([self.factor, self.factor], -1))
