...
from env.pysocialforce.utils import stateutils


class PedState:
    """Tracks the state of pedstrains and social groups"""

    def __init__(self, state, groups, config, radius, step_width):
    ...
    def state(self, state):
        ...
        self.ped_states = self._state.copy()

    def get_states(self):
        #return np.stack(self.ped_states), self.group_states
        return self.ped_states, self.group_states
    ...
    def groups(self, groups: List[List]):
        ...
        self.group_states = self._groups.copy()
    ...
    def obstacles(self, obstacles):
        ...
        if obstacles is None:
            ...
            for startx, endx, starty, endy in obstacles:
                samples = max(2, int(np.linalg.norm((startx - endx, starty - endy)) / 0.01))
                line = np.array(
                    list(
